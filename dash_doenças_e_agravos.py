...
print("[DASH] Carregando casos mensais...", flush=True)
...
# ── GeoJSON ───────────────────────────────────────────────────────────────────
print("[DASH] Carregando GeoJSON...", flush=True)
...
def criar_figura_mapa(z_vals, custom_data):
    fig = go.Figure(go.Choroplethmap(
        ...
        zmin=0, zmax=1,
        colorscale=[
            [0.0, "#F8FAFB"], [0.1, "#F8FAFB"], [0.11, "#BEE3F8"],
            [0.3, "#63B3ED"], [0.5, "#3182CE"], [0.7, "#2B6CB0"],
            [0.9, "#1A365D"], [0.91, "#C53030"], [1.0, "#C53030"],
        ],
        marker_opacity=0.95, marker_line_color="#8FA3B3", marker_line_width=0.5,
...
del _pivot, _z_values, _hover_dict, _customdata, df_mapa_raw, _mapa_busca_data
...
def _kpi(nome, valor, cor):
    return html.Div([
        html.P(f"{valor:,.0f}".replace(",", "."), style={"fontSize": 26, "fontWeight": 700, "color": "#fff", "margin": "0 0 4px 0"}),
        html.P(nome, style={"fontSize": 11, "fontWeight": 600, "color": "#fff", "margin": 0, "textTransform": "uppercase", "letterSpacing": "0.05em"}),
    ], style={"backgroundColor": cor, "borderRadius": 8, "padding": "18px 22px", "flex": 1, "minWidth": "200px"})

def _card(children):
    return html.Div(children, style={"backgroundColor": "#ffffff", "borderRadius": 8, "padding": "20px 24px", "border": "1px solid #e2e8f0", "flex": 1})
...
fig_barras = go.Figure(go.Bar(
    x=df_sorted["total_casos"], y=[nomes_exibicao[d] for d in df_sorted["doenca"]],
    orientation="h", marker_color=[COR_POR_DOENCA[d] for d in df_sorted["doenca"]],
    hovertemplate="<b>%{y}</b><br>Casos: %{x:,.0f}<extra></extra>",
))
fig_barras.update_layout(
    margin=dict(l=10, r=20, t=10, b=40), plot_bgcolor="#ffffff", paper_bgcolor="#ffffff",
    ...
    yaxis=dict(showgrid=False, linecolor="#e0e0e0"), height=420,
)
...
kpis = [_kpi(nomes_exibicao[row['doenca']], row['total_casos'], COR_POR_DOENCA[row['doenca']]) for _, row in df_total.iterrows()]
    ...
    children=[
        html.Div([
            html.Div([
                ...
                html.Div([
                    html.P("FUNASA", style={"fontSize": 10, "color": "#8BAFC8", "margin": "0 0 2px 0", "letterSpacing": "0.1em", "fontWeight": 600}),
                    html.H1("Doenças e Agravos — Casos Notificados", style={"fontSize": 22, "fontWeight": 700, "color": "#fff", "margin": 0}),
                    html.P("Total de casos notificados por doença (2007–2025) · Fonte: SINAN/DATASUS", style={"fontSize": 12, "color": "#8BAFC8", "margin": "4px 0 0 0"}),
                ], id="header-titulo", style={"flex": 1}),
            ], style={"display": "flex", "alignItems": "center", "flex": 1}),
            html.Div([
                html.P(f"{len(doencas)}", style={"fontSize": 28, "fontWeight": 700, "color": "#fff", "margin": 0, "textAlign": "center"}),
                html.P("DOENÇAS", style={"fontSize": 10, "color": "#8BAFC8", "margin": 0, "textAlign": "center", "letterSpacing": "0.05em"}),
            ], id="header-badge", style={"backgroundColor": "#2d4a6b", "borderRadius": 8, "padding": "12px 20px"}),
        ], id="header-container", style={"backgroundColor": COR_HEADER, "padding": "20px 32px", "display": "flex", "alignItems": "center", "justifyContent": "space-between"}),

        html.Div(id="conteudo-principal", style={"padding": "24px 32px"}, children=[
            html.P("Total de casos notificados por doença no período de 2007 a 2025", style={"fontSize": 14, "fontWeight": 600, "color": "#2d3748", "marginBottom": 12}),
                ...
                _card([
                    ...
                    html.Div([
                        html.Div([
                            ...
                            dcc.Dropdown(
                                id="mapa-filtro-doenca", options=[{"label": nomes_exibicao[d], "value": d} for d in doencas],
                                value=[], multi=True, placeholder="Todas as doenças", style={"width": "320px", "fontSize": 13},
                            ),
                        ]),
                        html.Div([
                            html.Label("Ano Início", style={"fontSize": 11, "fontWeight": 600, "color": "#4a5568"}),
                            dcc.Dropdown(
                                id="mapa-filtro-ano-inicio", options=[{"label": a, "value": a} for a in anos],
                                value=anos[0], clearable=False, style={"width": "100px", "fontSize": 13},
                            ),
                        ]),
                        html.Div([
                            html.Label("Ano Fim", style={"fontSize": 11, "fontWeight": 600, "color": "#4a5568"}),
                            dcc.Dropdown(
                                id="mapa-filtro-ano-fim", options=[{"label": a, "value": a} for a in anos],
                                value=anos[-1], clearable=False, style={"width": "100px", "fontSize": 13},
                            ),
                        ]),
                        html.Div([
                            html.Label("Buscar município", style={"fontSize": 11, "fontWeight": 600, "color": "#4a5568"}),
                            dcc.Input(
                                id="mapa-busca-municipio", type="text", placeholder="Digite o nome...", debounce=True,
                                style={"width": "220px", "fontSize": 13, "padding": "6px 12px", "border": "1px solid #e2e8f0", "borderRadius": 6},
                            ),
                        ]),
                    ], id="mapa-filtros-container", style={"display": "flex", "gap": 16, "marginTop": 12, "marginBottom": 8, "flexWrap": "wrap", "alignItems": "flex-end"}),
                    
                    # Mapa com ID fixo para o clique funcionar corretamente
                    html.Div(
                        id="mapa-municipios-container",
                        children=[
                            dcc.Graph(
                                id="mapa-municipios",
                                figure=fig_mapa,
                ...
                _card([
                    ...
                    html.Div([
                        html.Div([
                            ...
                            dcc.Dropdown(id="filtro-doenca", options=[{"label": nomes_exibicao[d], "value": d} for d in doencas], value="dengue", clearable=False, style={"width": "180px", "fontSize": 13}),
                        ]),
                        html.Div([
                            html.Label("Região", style={"fontSize": 11, "fontWeight": 600, "color": "#4a5568"}),
                            dcc.Dropdown(id="filtro-regiao", options=[{"label": "Todas", "value": "Todos"}] + [{"label": r, "value": r} for r in regioes], value="Todos", clearable=False, style={"width": "160px", "fontSize": 13}),
                        ]),
                        html.Div([
                            html.Label("UF", style={"fontSize": 11, "fontWeight": 600, "color": "#4a5568"}),
                            dcc.Dropdown(id="filtro-uf", options=[{"label": "Todas", "value": "Todos"}] + [{"label": uf, "value": uf} for uf in ufs], value="Todos", clearable=False, style={"width": "120px", "fontSize": 13}),
                        ]),
                        html.Div([
                            html.Label("Ano", style={"fontSize": 11, "fontWeight": 600, "color": "#4a5568"}),
                            dcc.Dropdown(id="filtro-ano", options=[{"label": "Todos", "value": "Todos"}] + [{"label": a, "value": a} for a in anos], value="Todos", clearable=False, style={"width": "120px", "fontSize": 13}),
                        ]),
                        html.Div([
                            html.Label("Mês", style={"fontSize": 11, "fontWeight": 600, "color": "#4a5568"}),
                            dcc.Dropdown(id="filtro-mes", options=[{"label": "Todos", "value": "Todos"}] + [{"label": NOME_MES[m], "value": m} for m in meses], value="Todos", clearable=False, style={"width": "140px", "fontSize": 13}),
                        ]),
                    ], id="filtros-container", style={"display": "flex", "gap": 16, "marginTop": 16, "marginBottom": 20, "flexWrap": "wrap"}),

                    html.Div([
                        html.Div(id="grafico-top10", style={"flex": 1}),
                        html.Div([
                            html.Div(id="tabela-municipios", style={"maxHeight": "420px", "overflowY": "auto", "borderLeft": "1px solid #e2e8f0", "paddingLeft": "20px"}),
                            html.P(id="tabela-info", style={"fontSize": 11, "color": "#9ca3af", "margin": "8px 0 0 0"}),
...
def atualizar_municipios(doenca_sel, regiao_sel, uf_sel, ano_sel, mes_sel):
    ...
    if df.empty:
        ...
        fig_vazio.update_layout(
            ...
            annotations=[{"text": "Nenhum dado encontrado", "showarrow": False, "font": {"size": 14, "color": "#9ca3af"}}],
            height=380,
        )
    ...
    fig.update_layout(
        ...
        yaxis=dict(showgrid=False), height=380,
    )
    ...
    for i, (_, row) in enumerate(top50.iterrows()):
        ...
        td = {"padding": "7px 12px", "fontSize": "12px", "borderBottom": "1px solid #f0f0f0", "backgroundColor": bg}
        rows_html.append(html.Tr([
            ...
            html.Td(f"{row['total_casos']:,}".replace(",", "."), style={**td, "fontWeight": "600", "textAlign": "right"}),
        ]))

    tabela = html.Table([html.Thead(header), html.Tbody(rows_html)], style={"width": "100%", "borderCollapse": "collapse"})
    total = len(df)
